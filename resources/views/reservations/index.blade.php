                    ...
                    <div class="mb-3 mb-md-0">
                        @if (request('history'))
                            ...
                            <p class="text-muted mb-0">
                                {{ __('messages.view_past_reservations') ?? 'View completed and past reservations' }}</p>
                        @else
                            <h1 class="h2 mb-2 text-gray-800">{{ __('messages.reservations') }}</h1>
                            <p class="text-muted mb-0">
                                {{ __('messages.manage_current_reservations') ?? 'Manage upcoming and current reservations' }}
                            </p>
                        @endif
                    </div>
                    ...
                    <div class="col-md-4 mb-3 mb-md-0">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <p class="text-muted mb-0">{{ __('messages.total_reservations') ?? 'Total Reservations' }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-8">
                        <form id="filterForm" action="{{ route('reservations.index') }}" method="GET">
                            <div class="row g-3">
                                ...
                                <div class="col-md-3">
                                    <label for="date_from"
                                        class="form-label small fw-medium">{{ __('messages.date_from') ?? 'Date From' }}</label>
                                    <input type="date" class="form-control form-control-sm" id="date_from"
                                        name="date_from" value="{{ request('date_from') }}" onchange="this.form.submit()">
                                </div>
                                <div class="col-md-3">
                                    <label for="date_to"
                                        class="form-label small fw-medium">{{ __('messages.date_to') ?? 'Date To' }}</label>
                                ...
                                <div class="col-md-4">
                                    <label for="search"
                                        class="form-label small fw-medium">{{ __('messages.search') ?? 'Search by Name or Phone' }}</label>
                                    <input type="text" class="form-control form-control-sm" id="search" name="search"
                                        value="{{ request('search') }}"
                                        placeholder="{{ __('messages.search_placeholder') ?? 'Enter name or phone...' }}">
                                </div>
                                ...
                                <table class="table table-hover mb-0">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            @can(['reservations_paid', 'reservations_edit', 'reservations_show',
                                                'reservations_delete'])
                                                <th class="text-center" style="width: 160px;">{{ __('messages.actions') }}
                                                </th>
                                            @endcan
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @forelse($reservations as $key => $reservation)
                                            <tr
                                                class="reservation-row {{ $reservation->status === ReservationStatus::WAITING && Carbon::parse($reservation->date)->isToday() ? 'today-reservation' : '' }}">
                                                <td class="ps-4 fw-semibold">
                                                    @if (
                                                        $reservation->status === ReservationStatus::WAITING &&
                                                            Carbon::parse($reservation->date)->isToday() &&
                                                            auth()->user()->can('reservations_apply'))
                                                        <a href="{{ route('reservations.applyPage', $reservation->id) }}"
                                                            class="text-decoration-none text-primary"
                                                            data-bs-toggle="tooltip"
                                                            title="{{ __('messages.apply_treatment') }}">
                                                ...
                                                <td>
                                                    <a href="{{ route('users.show', $reservation->user->id) }}"
                                                        ...
                                                        <div class="d-flex align-items-center">
                                                            <div
                                                                class="patient-avatar bg-primary text-white rounded-circle me-3">
                                                            ...
                                                            <div>
                                                                ...
                                                                @if ($reservation->user->phone)
                                                                    <small
                                                                        class="d-block text-muted">{{ $reservation->user->phone }}</small>
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </a>
                                                ...
                                                <td>
                                                    <span class="badge bg-light text-dark border">
                                                        {{ Carbon::parse($reservation->from)->format('h:i A') }} -
                                                        {{ Carbon::parse($reservation->to)->format('h:i A') }}
                                                    </span>
                                                </td>
                                                <td>
                                                    <span
                                                        class="fw-medium">{{ Carbon::parse($reservation->date)->format('M d, Y') }}</span>
                                                </td>
                                                <td class="text-end pe-4">
                                                    <span
                                                        class="badge bg-success bg-opacity-10 text-success fs-6 py-2 px-3">
                                                        ...
                                                        <div class="btn-group btn-group-sm" role="group">
                                                            ...
                                                            @if ($reservation->status == ReservationStatus::WAITING)
                                                                @can('reservations_edit')
                                                                    @if (auth()->user()->hasRole(['admin', 'secretary']) || $reservation->user_id == auth()->id())
                                                                        <a href="{{ route('reservations.edit', $reservation->id) }}"
                                                                            class="btn btn-outline-warning"
                                                                            data-bs-toggle="tooltip"
                                                                            title="{{ __('messages.edit_reservation') }}">
                                                                ...
                                                                @can('reservations_delete')
                                                                    <form
                                                                        ...
                                                                        <button type="submit"
                                                                            ...
                                                                            data-bs-toggle="tooltip"
                                                                            title="{{ __('messages.cancel_reservation') }}">
                                                            ...
                                                            @if ($reservation->status === ReservationStatus::TOPAY)
                                                                @can('reservations_paid')
                                                                    <form
                                                                        action="{{ route('reservations.paid', $reservation->id) }}"
                                                                        method="POST" class="d-inline">
                                                                        @csrf
                                                                        @method('put')
                                                                        <button type="submit"
                                            ...
                                            <tr>
                                                <td colspan="{{ auth()->user()->can('reservations_paid') || auth()->user()->can('reservations_edit') || auth()->user()->can('reservations_show') || auth()->user()->can('reservations_delete') ? 8 : 7 }}"
                                                    ...
                                                    <div class="empty-state">
                                                        ...
                                                        <h5 class="text-muted">
                                                            {{ __('messages.no_reservations_found') ?? 'No reservations found' }}
                                                        </h5>
                                                        <p class="text-muted mb-4">
                                                            {{ request('today')
                                                                ? __('messages.no_reservations_today') ?? 'No reservations scheduled for today'
                                                                : (request('history')
                                                                    ? __('messages.no_reservations_history') ?? 'No past reservations found'
                ...
                <div class="d-block d-lg-none">
                    <div class="row g-3">
                        @forelse($reservations as $key => $reservation)
                            <div class="col-12">
                                <div
                                    class="card reservation-card shadow-sm border-0 h-100
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="d-flex justify-content-between align-items-start mb-3">
                                            <div class="reservation-badge bg-primary text-white rounded-3 px-3 py-2">
                                                <small
                                                    class="fw-bold">#{{ $key + 1 + ($reservations->currentPage() - 1) * $reservations->perPage() }}</small>
                                            </div>
                                            <span
                                                class="badge {{ [
                                                    ReservationStatus::WAITING => 'bg-warning',
                                        ...
                                        <div class="d-flex justify-content-between align-items-center pt-3 border-top">
                                            <div class="price-display">
                                                <span
                                                    class="fw-bold text-success">${{ number_format($reservation->total_price, 2) }}</span>
                                            </div>
                                            <div class="action-buttons">
                                                ...
                                                @if ($reservation->status === ReservationStatus::TOPAY)
                                                    @can('reservations_paid')
                                                        <form action="{{ route('reservations.paid', $reservation->id) }}"
                                                            method="POST" class="d-inline">
                                                            @csrf
                                                            @method('put')
                                ...
                                <div class="card border-0 shadow-sm">
                                    <div class="card-body text-center py-5">
                                        <div class="empty-state">
                                            ...
                                            <h5 class="text-muted">
                                                {{ __('messages.no_reservations_found') ?? 'No reservations found' }}</h5>
                                            <p class="text-muted mb-4">
                                                {{ request('today')
                                                    ? __('messages.no_reservations_today') ?? 'No reservations scheduled for today'
                                                    : (request('history')
